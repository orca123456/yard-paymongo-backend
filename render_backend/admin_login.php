...
// Count helper
function countOrders($pdo, $status = null)
{
    if ($status === null) {
        $stmt = $pdo->query("SELECT COUNT(*) FROM preorders");
        return (int) $stmt->fetchColumn();
    }

    $stmt = $pdo->prepare("SELECT COUNT(*) FROM preorders WHERE order_status = :status");
    $stmt->execute([
        ':status' => $status
    ]);
...
$pendingCount = countOrders($pdo, 'pending');
$processingCount = countOrders($pdo, 'processing');
$deliveredCount = countOrders($pdo, 'delivered');
$cancelledCount = countOrders($pdo, 'cancelled');
